import { MessagesApiActions, selectMessagesEntities, selectSelectedChat } from '@angular-slack/data-access-messages';
...
export class ThreadsEffects {
  ...
  loadThreadsMessages$ = createEffect(() =>
    this.actions$.pipe(
      ...
      concatMap(({threads}) => threads.map((thread) => MessagesApiActions.init({
  ...
  this.actions$.pipe(
    ...
    concatLatestFrom((action) => [this.store.select(selectMessagesEntities).pipe(
      map(dictionary => dictionary[action.message.chatId])
